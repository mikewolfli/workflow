...
Value_Pool* Rfc_Communication::wrapVariable(RFCTYPE typ, RFC_FUNCTION_HANDLE functionHandle, SAP_UC* cName, unsigned int cLen, RFC_TYPE_DESC_HANDLE typeDesc, bool rstrip)
{
  ...
  switch(typ) {
    ...
    case RFCTYPE_TIME:
        {
      resultValue = new Value_Pool(1,false);
      line_value = resultValue->Get_Value(0);
      wxString str = wrapField(typ, functionHandle, cName, cLen,rstrip);
      line_value->cols = &str;
      break;
    }
    default:
...
Value_Pool * Rfc_Communication::get_same_value_pool(wxString str, int i_col, Value_Pool* p_result)
{
