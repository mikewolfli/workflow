...
BEGIN_EVENT_TABLE(wxTreeListCtrl, wxControl)
    ...
    EVT_MOTION(wxTreeListCtrl::OnMouseMotion)
END_EVENT_TABLE();
...
void  wxTreeListCtrl::OnMouseMotion(wxMouseEvent& event)
{
   int id = event.GetEventType();
   int i_count = this->GetColumnCount();
  ...
  for(i=0;i<i_count;i++)
   {
       tmpWidth = this->GetColumnWidth(i);
      ...
      item_id = this->HitTest(pt, flags);


   if( item_id.IsOk() && col > -1)
	   toolTip = this->GetItemText(item_id, col);
    else
        toolTip.Empty();

   if( toolTip.Length() > 0 && item_id.IsOk())
   {
